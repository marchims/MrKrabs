        ...
        if sell_guidance == True and buy_guidance == False:
            ...
            if amt*(1-self.fee_amt) >= self.min_trade_amt:
                ...
                if self.trading_enabled == False:
                    ...
                    result = self.client.create_test_order(symbol=self.pair,side=SIDE_SELL,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/bid,2),price='{:.8f}'.format(bid),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                    ...
                    result = self.client.create_order(symbol=self.pair,side=SIDE_SELL,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/bid,2),price='{:.8f}'.format(bid),newOrderRespType=ORDER_RESP_TYPE_RESULT)
            ...
            if amt >= self.min_trade_amt:
                ...
                if self.trading_enabled == False:
                    ...
                    result = self.client.create_test_order(symbol=self.pair,side=SIDE_BUY,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/ask,2),price='{:.8f}'.format(ask),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                    ...
                    # Do something like initiate a trade
                    result = self.client.create_order(symbol=self.pair,side=SIDE_BUY,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/ask,2),price='{:.8f}'.format(ask),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                    coin_amt = float(result['executedQty'])
                    if coin_amt > 0:
                        amt = coin_amt*float(result['price'])
                        if self.bias - amt < 0:
                ...
                if (bid-self.mean_buy_price)/self.mean_buy_price >= self.margin + self.fee_amt:
                    ...
                    if amt*(1-self.fee_amt) >= self.min_trade_amt:
                        ...
                        if self.trading_enabled == False:
                            ...
                            # Do something like initiate a trade
                            result = self.client.create_order(symbol=self.pair,side=SIDE_SELL,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/bid+self.coin_precision,2),price='{:.8f}'.format(bid),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                            coin_amt = float(result['executedQty'])
                            if coin_amt > 0:
                                amt = coin_amt*float(result['price'])
                                if self.bias + amt*(1-self.fee_amt) > 0:
                ...
                if (self.mean_sell_price-ask)/self.mean_sell_price >=self.margin + self.fee_amt:
                    ...
                    if amt >= self.min_trade_amt:
                        if self.trading_enabled == False:
                            ...
                            result = self.client.create_test_order(symbol=self.pair,side=SIDE_BUY,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/ask,2),price='{:.8f}'.format(bid),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                            self.base_amt = self.base_amt - amt
                            self.coin_amt = self.coin_amt + amt/ask*(1-self.fee_amt)
                            ...
                            # Do something like initiate a trade
                            result = self.client.create_order(symbol=self.pair,side=SIDE_BUY,type=ORDER_TYPE_LIMIT,timeInForce=TIME_IN_FORCE_IOC,quantity=round(amt/ask+self.coin_precision,2),price='{:.8f}'.format(ask),newOrderRespType=ORDER_RESP_TYPE_RESULT)
                            coin_amt = float(result['executedQty'])
